  ...
  <view class="select-container" v-show="show" @touchmove.stop.prevent>
    ...
    <view class="select-box" :class="activeClass ? 'select-box-show' : ''">
      ...
      <view class="body-warp">
        <scroll-view class="body" scroll-y="true" :scroll-into-view="block">
          ...
          <view
            ...
            :key="item[valueName]"
            :id="'block' + item[valueName]"
            @tap="onSelected(index)"
          >
            <view class="label">{{item[labelName]}}

            <text v-show="selectedArr[index]" class="u-m-l-34  fortune icon-fortuneduihao1"></text>
            </view>
          </view>
        </scroll-view>
      </view>
...
export default {
  ...
  watch: {
    async value(newVal) {
      this.show = newVal;
      // 滚动到第一个
      this.block = 'block' + this.defaultSelected[0]
      await this.$nextTick();
  ...
  methods: {
    ...
    onConfirm() {
      ...
      if (this.mode === "multiple") {
        this.$emit("confirm", selectedData, this.formKey, this.labelName, this.valueName);
      } else {
        let backData = selectedData[0] || {};
        this.$emit("confirm", backData, this.formKey, this.labelName, this.valueName);
      }
    }
  }
};
